...
logger = logging.getLogger("loadout.http")
...
def offline() -> bool:
    ...
    return env_flag("LOADOUT_OFFLINE")
...
def polite_get(
    ...
    *,
    ...
    # An unauthenticated GitHub API allows 60 requests/hour; a token raises it
    # to 5000 and costs the user nothing to set.
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            resp = requests.get(
                url, timeout=timeout, headers=merged_headers, stream=stream
            )
        ...
        if attempt < max_attempts:
            # Jitter to spread retries; not a security decision.
            sleep_for = min(MAX_BACKOFF, backoff + random.random())  # noqa: S311
